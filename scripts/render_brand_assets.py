...
def make_background(width: int, height: int) -> Image.Image:
    img = Image.new("RGB", (width, height), "#e7f5ee")
    draw = ImageDraw.Draw(img)

    for y in range(height):
        blend = y / max(height - 1, 1)
        r = int(231 - (231 - 9) * blend)
        g = int(245 - (245 - 34) * blend)
        b = int(238 - (238 - 29) * blend)
        draw.line([(0, y), (width, y)], fill=(r, g, b))

    for box, fill in [
        ((-120, -80, width // 2, height // 2), (16, 185, 129)),
        ((width // 2, 0, width + 180, height // 2 + 100), (12, 120, 102)),
        ((width // 3, height // 2, width + 140, height + 100), (7, 41, 34)),
    ]:
...
def draw_badge(img: Image.Image, center: tuple[int, int], radius: int, top: str, bottom: str) -> None:
    draw = ImageDraw.Draw(img)
    cx, cy = center
    draw.ellipse((cx - radius, cy - radius, cx + radius, cy + radius), fill="#edf8f2", outline="#18473d", width=5)
    draw.ellipse((cx - radius + 16, cy - radius + 16, cx + radius - 16, cy + radius - 16), outline="#10b981", width=4)
    for i in range(18):
        ...
        draw.line((x1, y1, x2, y2), fill="#10b981", width=3)
    ...
    draw.text((cx - (tw[2] - tw[0]) / 2, cy - 28), top, font=title_font, fill="#18473d")
    draw.text((cx - (bw[2] - bw[0]) / 2, cy + 10), bottom, font=sub_font, fill="#4b665f")
...
def render_hero(data: dict) -> None:
    ...
    draw.text((126, 148), data["kicker"].upper(), font=kicker_font, fill="#10b981")
    ...
    chips = [
        f"{data['stats']['lessonCount']} module curriculum",
        f"{data['stats']['runtime']} total runtime",
    ]
    x = 126
    y = body_box[3] + 24
    for chip in chips:
    ...
    draw.text((974, 150), "Ecosystem Lens", font=font(DISPLAY_BOLD, 32), fill="#f4f7f6")
    for idx, pillar in enumerate(
        [("Codex", "Code generation and refactors"), ("Gemini", "Reasoning and long-context work"), ("Superset", "MCP orchestration layer")]
    ):
        top = 206 + idx * 68
...
def render_social_card(data: dict) -> None:
    ...
    draw.text((150, 164), data["kicker"].upper(), font=font(SANS_FONT, 28), fill="#10b981")
    title_text = wrap(data["title"], 16)
    ...
    draw_badge(img, (1298, 452), 126, "AI", "Ecosystem")
...
def render_slates(data: dict) -> None:
    ...
    for kind, title, body, filename in [
        (
            "intro",
            data["title"],
            "Install the stack, learn routing logic, and run coordinated multi-model workflows across Codex, Gemini, and Superset.",
            BRAND_DIR / "intro-slate.png",
        ),
        (
            "outro",
            "Next Step",
            "Use the one-pager and trainee guide to launch your implementation sprint and finalize your AI Ecosystem Design Document.",
        ...
        draw.text((132, 142), kind.upper(), font=font(SANS_FONT, 22), fill="#10b981")
        ...
        draw_badge(img, (1038, 518), 88, "AI", "Stack")
    ...
    for index, module in enumerate(data["modules"], start=1):
        ...
        draw.text((132, 136), module["orderLabel"].upper(), font=font(SANS_FONT, 22), fill="#10b981")
        ...
        draw_badge(img, (1048, 508), 94, "Module", f"{index:02d}")
